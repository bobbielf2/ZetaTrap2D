function u = HelmSLPn(ka,t,s,dens,ord)
...
if (numel(s.x)==numel(t.x)) && max(abs(s.x-t.x))<1e-14 % self eval
    ...
    A = -pi*0.5i*ka * (A+A.') .* real(conj(s.nx).*d)./r;
    ...
    if nargin < 4 || isempty(dens) %output matrix
        ...
        u = u + circ * [c(K+1:-1:2),c(2:K+1)]'; % loca correction
    end
else
    % build BIO matrix (note scaled by 2*pi)
    A = -pi*0.5i*ka * besselh(1,ka*r) .* real(conj(t.nx).*d)./r;
    if nargin < 4 || isempty(dens) %output matrix
        u = bsxfun(@times, A, (1/2/pi) * s.w(:).'); % speed wei & prefactor
    else
        ttau = (1/2/pi)*dens.*s.w;
