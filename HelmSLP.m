function u = HelmSLP(ka,t,s,dens,ord)
...
if nargin < 5 || isempty(ord), ord = 16; end
...
if (numel(s.x)==numel(t.x)) && max(abs(s.x-t.x))<1e-14 % self eval
    % build BIO matrix using symmetry (note scaled by 2*pi)
    A = (pi*.5i) * triu(besselh(0,triu(ka*r,1)),1);
    A = A.' + A;
    ...
    cg = pi*0.5i - (log(ka/2)+double(eulergamma));  % constant term
    A(diagind(A)) = cg-log(s.w);                    % diag limit
    ...
    if nargin < 4 || isempty(dens) %output matrix
        A(ind) = A(ind) + [c(K+1:-1:2),c].*J; % loc corr
        %A = A + toeplitz([c,zeros(1,N-2*K-1),c(K+1:-1:2)]).*besselj(0,ka*r);
        u = bsxfun(@times, A, (1/2/pi) * s.w(:).'); % speed wei & prefactor
    else
        ttau = (1/2/pi)*dens.*s.w;
        ...
        u = u + circ * [c(K+1:-1:2),c]'; % loca correction
    end
else
    A = pi*0.5i * besselh(0,ka*r); % build BIO matrix (note scaled by 2*pi)
